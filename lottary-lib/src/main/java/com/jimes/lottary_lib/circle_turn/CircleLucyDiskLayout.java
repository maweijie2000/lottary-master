...
public class CircleLucyDiskLayout extends RelativeLayout {
    ...
    private CircleLucyDiskView rotatePan;
    ...
    public CircleLucyDiskLayout(Context context) {
        this(context, null);
    }

    public CircleLucyDiskLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleLucyDiskLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= SizeUtils.dip2px(context, 10) * 2;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child instanceof CircleLucyDiskView) {
                rotatePan = (CircleLucyDiskView) child;
    ...
    private void drawSmallCircle(boolean FirstYellow) {
        int pointDistance = radius - SizeUtils.dip2px(context, 10);
        for (int i = 0; i <= 360; i += 20) {
            int x = (int) (pointDistance * Math.sin(SizeUtils.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(SizeUtils.change(i))) + CircleY;

            if (FirstYellow)
                canvas.drawCircle(x, y, SizeUtils.dip2px(context, 4), yellowPaint);
            else
                canvas.drawCircle(x, y, SizeUtils.dip2px(context, 4), whitePaint);
